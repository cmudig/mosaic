...
view = vg.vconcat(
    vg.hconcat(
        ...
        vg.menu(source="proteins", column="gradient_decay_function", label="Gradient Decay", bind=query),
        vg.menu(source="proteins", column="gradient_scale", label="Gradient Scale", bind=query)
    ),
    vg.vspace("1.5em"),
    vg.hconcat(
        vg.plot(
            vg.rect_y(data="proteins", filter_by=query, x=vg.bin("plddt_total", steps=60), y=vg.count(), z="version", fill="version", order="z", reverse=True, inset_left=0.5, inset_right=0.5),
            vg.width(600),
            vg.height(55),
            vg.x_axis(None),
            vg.y_axis(None),
    ...
    vg.hconcat(
        vg.plot(
            vg.frame(stroke="#ccc"),
            vg.raster(data="proteins", filter_by=query, x="plddt_total", y="pae_interaction", fill="version", pad=0),
            vg.interval_xy(bind=query, brush=vg.brush(stroke="currentColor", fill="transparent")),
            vg.dot(data="proteins", filter_by=point, x="plddt_total", y="pae_interaction", fill="version", stroke="currentColor", stroke_width=0.5),
            vg.name("scatter"),
        ...
        vg.plot(
            vg.rect_x(data="proteins", filter_by=query, x=vg.count(), y=vg.bin("pae_interaction", steps=60), z="version", fill="version", order="z", reverse=True, inset_top=0.5, inset_bottom=0.5),
            vg.width(55),
            vg.height(450),
            vg.x_axis(None),
            vg.y_axis(None),
    ...
    vg.vspace("1em"),
    vg.table_input(bind=point, filter_by=query, source="proteins", columns=[
        "version",
        "pae_interaction",
        "plddt_total",
        "noise",
