...
export async function cache_hit_graph(jsonfile, size, lru_cache, {width, height} = {}) {
...
export async function cache_repeat_hit_graph(jsonfile, size, lru_cache, {width, height} = {}) {
    let x_axis = Array.from({ length: 11 }, (_, i) => Math.floor((i * size) / 10));
    let data = await Promise.all(
        x_axis.map(async (xValue) => {
          const rate = await cache_repeat_hit(jsonfile, lru_cache(xValue));
          return { x: xValue, y: rate };
        })
    );
