export function latency_based_cache(cache_size = 10 * 1024 * 1024) {
    ...
    function evict() {
        let evictKey = null;
        let minLatency = Infinity; // Query with lowest latency is evicted first

        for (const [key, entry] of cache) {
            const latency = parseFloat(entry.value.Latency) || 0;
