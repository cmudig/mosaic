...
view = vg.vconcat(
    vg.menu(label="Sample", bind=sample, options=[vg.option("Full Data", value="flights10m"), vg.option("10% Sample", value="flights10p"), vg.option("5% Sample", value="flights5p"), vg.option("1% Sample", value="flights1p")]),
    vg.vspace(10),
    vg.plot(
        vg.raster(data=sample, x="time", y="delay", pixel_size=4, pad=0, image_rendering="pixelated"),
        vg.regression_y(data=sample, x="time", y="delay", stroke="gray"),
        vg.regression_y(data=sample, filter_by=query, x="time", y="delay", stroke="firebrick"),
